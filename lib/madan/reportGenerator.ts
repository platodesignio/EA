...
export function generateMarkdownReport(
  ...
  return `# MADAN Audit Report — ${scenario.title}
**Generated:** ${now}
**System Type:** ${scenario.systemType}
**Final Judgment:** ${result.finalJudgment}
**DCR Score:** ${dcrSign}${result.dcr.toFixed(2)} / ±5.00

---

## Executive Summary

The Multi-Agent Dialectical Audit Network (MADAN) conducted a full twelve-agent audit of **${scenario.title}**. The audit produced a Dialectical Correctness Rating (DCR) of **${dcrSign}${result.dcr.toFixed(2)}**, yielding a final directional judgment of **${result.finalJudgment}**.

The audit identified ${result.freedomClosureRisks.length} freedom-closure risks, ${result.mainContradictions.length} main contradictions across agent positions, and a reachable-state impact score of ${result.reachableStateImpact > 0 ? "+" : ""}${result.reachableStateImpact.toFixed(1)}.
...
*Generated by MADAN — Multi-Agent Dialectical Audit Network*
*DDAT Studio | ${now}*
`
}
...
export function generateJSONExport(
  ...
  const exportData = {
    metadata: {
      ...
      auditSystem: "MADAN — Multi-Agent Dialectical Audit Network",
      version: "1.0.0",
    },
    scenario,
    auditResult: {
